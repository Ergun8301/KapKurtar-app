...
const ClientDashboardPage = () => {
  ...
  const ReservationCard = ({ reservation }: { reservation: Reservation }) => {
    ...
    return (
      <div className={`bg-white rounded-xl shadow-sm border overflow-hidden transition-all hover:shadow-lg ${
        isPending ? 'border-orange-300' : 
        ...
        <div className="md:hidden">
          {/* Header Mobile */}
          <div className="bg-gradient-to-r from-gray-50 to-gray-100 p-3 border-b flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">{reservation.merchant_city}</p>
              </div>
            </div>
            {isPending && (
          ...
          <div className="p-3">
            <div className="flex gap-3 mb-3">
              ...
              <div className="flex-1">
                <h3 className="font-bold text-gray-900 text-sm mb-1">{reservation.offer_title}</h3>
            ...
            {isPending && (
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-xs font-medium transition-colors"
                >
            ...
            {!isPending && (
              <button
                onClick={() => handleArchive(reservation.reservation_id)}
                className="w-full flex items-center justify-center gap-1 px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-xs font-medium transition-colors"
              >
        ...
        <div className="hidden md:grid md:grid-cols-12 md:gap-4 md:p-4">
          {/* GAUCHE : Marchand (3 colonnes) */}
          <div className="col-span-3 flex flex-col items-center justify-center text-center border-r border-gray-100">
            {reservation.merchant_logo_url ? (
              <img
                src={reservation.merchant_logo_url}
                alt={reservation.merchant_name}
                className="w-16 h-16 rounded-full object-cover border-2 border-gray-200 shadow-sm mb-2"
                crossOrigin="anonymous"
                referrerPolicy="no-referrer"
                onError={(e) => {
                  (e.currentTarget as HTMLImageElement).style.display = 'none';
              ...
              <div className="w-16 h-16 rounded-full bg-green-500 flex items-center justify-center text-white font-bold text-2xl mb-2">
                {reservation.merchant_name[0]}
              </div>
            )}
            <p className="font-bold text-sm text-gray-900 mb-1">{reservation.merchant_name}</p>
            <p className="text-xs text-gray-500 flex items-center gap-1">
              <MapPin className="w-3 h-3" />
              {reservation.merchant_city}
            </p>
            {reservation.merchant_street && (
              <p className="text-xs text-gray-400 mt-1 line-clamp-2 px-2">
                {reservation.merchant_street}
              </p>
            )}
          </div>

          {/* CENTRE : Produit (6 colonnes) */}
            ...
            <div className="flex-1 flex flex-col justify-between">
              <div>
                <div className="flex items-start justify-between mb-2">
                  <h3 className="font-bold text-gray-900 text-base">{reservation.offer_title}</h3>
                  {isPending && (
                    <span className="bg-orange-100 text-orange-700 text-xs font-semibold px-2 py-1 rounded-full ml-2">
                      En attente
                    </span>
                  )}
                  {isCompleted && (
                  ...
                  {isExpired && (
                    <span className="bg-gray-100 text-gray-700 text-xs font-semibold px-2 py-1 rounded-full ml-2">
                      Expirée
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-2xl font-bold text-green-600">{reservation.total_price.toFixed(2)}€</span>
          ...
          {/* DROITE : Actions (3 colonnes) */}
          <div className="col-span-3 flex flex-col gap-2 justify-center border-l border-gray-100 pl-4">
            {isPending ? (
              <>
                <button
                  onClick={() => handleGetDirections(reservation.merchant_lat, reservation.merchant_lng)}
                  className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm font-medium transition-colors shadow-sm"
                >
                ...
                {reservation.merchant_phone && (
                  <a
                    href={`tel:${reservation.merchant_phone}`}
                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-green-500 hover:bg-green-600 text-white rounded-lg text-sm font-medium transition-colors shadow-sm"
                  >
