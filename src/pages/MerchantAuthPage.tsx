...
const MerchantAuthPage = () => {
  ...
  // ➊ Si retour OAuth sur cette page: fixe le rôle=merchant puis route
  useEffect(() => {
    (async () => {
      if (!initialized) return;
      if (!user) return;

      // Si le rôle n'est pas encore "merchant", le fixer
      if (role !== 'merchant') {
        await supabase.rpc('set_role_for_me', { p_role: 'merchant' });
      }

      // Quand profil dispo, router
      if (role === 'merchant' && profile) {
        const { data: products } = await supabase
          .from('offers').select('id').eq('merchant_id', profile.id).limit(1);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        if (!formData.companyName.trim()) throw new Error("Le nom de l'entreprise est requis");

        const { error: authError } = await supabase.auth.signUp({
          email: formData.email,
          password: formData.password,
          options: { data: { source: 'merchant', company_name: formData.companyName } }
        });
        if (authError) throw authError;
        // après signUp email, session peut être déjà ouverte selon config
        await supabase.rpc('set_role_for_me', { p_role: 'merchant' });
      }
    } catch (err) {
  ...
  const handleGoogleAuth = async () => {
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/merchant/auth`, // revenir ici
    ...
    <div className="min-h-screen bg-[#FAFAF5] flex flex-col">
      <div className="flex-1 flex items-center justify-center py-8 px-4">
        <div className="w-full max-w-md space-y-6">
          <button onClick={() => navigate('/')} className="inline-flex items-center text-[#FF6B35] hover:text-[#e55a28] font-medium mb-4">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-900 mb-2">{mode === 'login' ? 'Espace Commerçant' : 'Devenez Partenaire'}</h1>
            <p className="text-gray-600">{mode === 'login' ? 'Gérez vos offres et réduisez le gaspillage' : 'Rejoignez SEPET et valorisez vos invendus'}</p>
          </div>

          <div className="bg-white rounded-3xl shadow-xl p-6 space-y-6">
            <div className="flex bg-gray-100 rounded-2xl p-1">
              <button onClick={() => setMode('login')} className={`flex-1 py-3 font-semibold rounded-xl ${mode === 'login' ? 'bg-white text-[#FF6B35] shadow-md' : 'text-gray-500'}`}>Connexion</button>
              <button onClick={() => setMode('register')} className={`flex-1 py-3 font-semibold rounded-xl ${mode === 'register' ? 'bg-white text-[#FF6B35] shadow-md' : 'text-gray-500'}`}>Inscription</button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" name="companyName" value={formData.companyName} onChange={handleInputChange}
                      ...
                      placeholder="Votre commerce" required={mode === 'register'} />
                  </div>
                </div>
              )}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" name="email" value={formData.email} onChange={handleInputChange}
                    className="w-full pl-12 pr-4 py-4 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-[#FF6B35] focus:border-transparent text-base"
                    placeholder="commerce@example.com" required />
                </div>
              </div>
              ...
              <button type="submit" disabled={isLoading}
                ...
                {isLoading ? (<div className="flex items-center justify-center"><div className="w-6 h-6 border-2 border-white border-t-transparent rounded-full animate-spin mr-2"></div>Chargement...</div>) : (mode === 'login' ? 'Se connecter' : 'Créer mon espace')}
              </button>
            </form>
