...
const MerchantInfoPage = () => {
  ...
  const testimonials = [
    {
      ...
      quote: 'TILKAPP helped us reduce waste by 60% while attracting amazing new customers. The professional partnership has been transformative for our business.',
      rating: 5,
      revenue: '+€2,400/month'
    },
    {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="text-center text-white px-4 max-w-4xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button
                ...
                className="bg-tilkapp-orange text-white px-8 py-4 rounded-lg font-semibold hover:bg-orange-600 transition-colors inline-flex items-center text-lg shadow-lg"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {benefits.map((benefit, index) => (
              <div key={index} className="text-center p-8 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                ...
                <div className="text-sm font-semibold text-orange-600 bg-orange-50 px-4 py-2 rounded-full inline-block">
                  {benefit.stats}
                </div>
              </div>
            ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {features.map((feature, index) => (
                  <div key={index} className="flex items-center space-x-3">
                    <CheckCircle className="w-5 h-5 text-tilkapp-orange flex-shrink-0" />
            ...
            {testimonials.map((testimonial, index) => (
              <div key={index} className="bg-gray-50 rounded-xl p-8 relative">
                <div className="flex items-center mb-4">
                  ...
                  <span className="ml-3 text-sm font-semibold text-orange-600 bg-orange-100 px-3 py-1 rounded-full">
                    {testimonial.revenue}
                  </span>
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="text-center">
              <div className="bg-tilkapp-orange text-white w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 text-2xl font-bold">
                1
              </div>
      ...
      {/* Final CTA Section */}
      <div className="py-24 bg-tilkapp-orange">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <button
            onClick={() => navigate('/merchant/auth')}
            className="bg-white text-orange-600 px-12 py-4 rounded-lg font-semibold hover:bg-gray-100 transition-colors inline-flex items-center text-xl shadow-lg"
          >
            <CheckCircle className="w-6 h-6 mr-3" />
            Register as Professional Partner
          </button>
          
          <p className="text-orange-200 mt-6">No setup fees • Professional support • Cancel anytime</p>
        </div>
      </div>
    </div>
  );
