...
import { OfferDetailsModal } from "../components/OfferDetailsModal"; // ← Utiliser le nouveau OfferDetailsModal_FINAL
...
export default function OffersPage() {
  ...
  useEffect(() => {
    ...
    // Amélioration gestion erreur GPS (Xiaomi fix)
    geolocate.on("error", (e) => {
      console.error("❌ Erreur géolocalisation:", e);

      // Message d'aide spécifique pour les appareils bloquants (Xiaomi MIUI, etc.)
      const isMobile = /Android|iPhone|iPad/i.test(navigator.userAgent);
      if (isMobile) {
        alert(
          "⚠️ Géolocalisation bloquée\n\n" +
          "Sur certains téléphones (Xiaomi, Huawei, etc.) :\n" +
          "1. Ouvrez Réglages > Autorisations\n" +
          "2. Trouvez votre navigateur\n" +
          "3. Activez 'Localisation'\n\n" +
          "Ou utilisez la barre de recherche ci-dessus."
        );
      }
    });
  ...
  // Fonction pour gérer le changement d'offre dans le modal
  const handleOfferChange = (newOffer: Offer) => {
    setSelectedOffer(newOffer);
  };

  // Fonction de réservation
  const handleReserve = async (offer: Offer) => {
    if (!user || !clientId) {
    ...
    try {
      ...
      if (error) {
        ...
        // Recharger les offres pour mettre à jour les stocks
  ...
  // Récupérer les infos marchand pour le modal
  const getMerchantInfo = (offer: Offer | null) => {
    if (!offer) return null;

    return {
      id: offer.merchant_id || "",
      business_name: offer.merchant_name,
      logo_url: offer.merchant_logo_url || null,
      address: [offer.merchant_street, offer.merchant_city, offer.merchant_postal_code]
        .filter(Boolean)
        .join(", ") || "Adresse non définie",
      phone: offer.merchant_phone || "",
      latitude: offer.offer_lat,
      longitude: offer.offer_lng,
    };
  };

  // Récupérer toutes les offres du marchand pour le modal
  const getCurrentMerchantOffers = () => {
    if (!selectedOffer) return [];

    const merchantId = selectedOffer.merchant_id || selectedOffer.merchant_name;
    return offers.filter(o =>
      (o.merchant_id || o.merchant_name) === merchantId && o.is_active
    ).map(o => ({
      id: o.offer_id,
      title: o.title,
  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100vh-100px)]">
      <div className="relative flex-1 border-r border-gray-200 h-1/2 md:h-full">
        ...
        {/* Slider rayon - FIX Z-INDEX de 1600 à 900 */}
        {viewMode === "nearby" && (
      ...
      {selectedOffer && (
        <OfferDetailsModal
          offer={{
            ...
            price_before: selectedOffer.price_before,
            price_after: selectedOffer.price_after,
            quantity: selectedOffer.quantity || 0,
            available_from: selectedOffer.available_from || "",
            available_until: selectedOffer.available_until || "",
            merchant_id: selectedOffer.merchant_id || "",
            is_active: selectedOffer.is_active || false,
          }}
          merchant={getMerchantInfo(selectedOffer)!}
          merchantOffers={getCurrentMerchantOffers()}
